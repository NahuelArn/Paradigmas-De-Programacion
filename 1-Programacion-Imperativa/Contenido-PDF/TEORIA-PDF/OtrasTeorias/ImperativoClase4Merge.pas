...
durante cada el año 2021. 
De cada préstamo se conoce el ISBN del libro, el número de socio, día y mes del 
préstamo y cantidad de días prestados. Implementar un programa con:
a.	Un módulo que lea préstamos y retorne en una estructura adecuada la información 
    de los préstamos *organizada por mes*. Para cada *mes*, los préstamos deben quedar 
    *ordenados por ISBN*. La lectura de los préstamos finaliza con ISBN -1.
b.	Un módulo recursivo que reciba la estructura generada en a. y muestre, 
    para cada mes, ISBN y numero de socio.
c.	Un módulo que reciba la estructura generada en a. y retorne una nueva estructura 
    con *todos* los préstamos *ordenados por ISBN*.
d.	Un módulo recursivo que reciba la estructura generada en c. y muestre todos los 
    ISBN y número de socio correspondiente.
e.	Un módulo que reciba la estructura generada en a. y retorne una nueva estructura 
    *ordenada ISBN*, donde cada *ISBN* aparezca una vez junto a la *cantidad* total de 
    veces que se prestó durante el año 2021.
...
Type rangoDias = 1.. 31;
     ...
     prestamoAGuardar = record    // Para guardar, NO contiene el mes
                          ISBN: integer;
                          numeroSocio: integer;
                          dia: rangoDias;
                          cantDias: integer;
...
{******* Inciso D: ImprimirEstructuraMerge *******}
...
begin
   ...
   if (l = nil ) 
     then writeln ('      Sin elementos')
     else imprimirLista (l); //REUSA módulo *imprimirLista* Inciso B
end;


{******* Inciso E: MergeAcumulador *******}
...
begin
  l:= nil;
  minimo (v, prestamoMin);  //REUSA módulo *minimo* Inciso C
  while (prestamoMin.ISBN <> valorGrande) do
  begin
...
{******* Inciso F: ImprimirEstructuraMergeAcumulador *******}
