...
procedure agregarAtras(var L,Ult: lista; v: venta1);
var
  nue: lista;
begin
  new(nue);
...
var i: rango3; 
  Ult: lista;
begin
  {L:= nil;}  //inicializaste en el program principal
  // for i:= dimL downto 1 do Por q el downto ???
  for i:= 1 to dimL do
    if Cumple (v[i].codigoP) then 
      begin
        agregarAtras(L,Ult,L^.dato);
      end;
      // AgregarAdelante (L, L^.dato.codigo,L^.dato.cantidad);  //es un agregar atras, fijate q tu vector ya esta ordenado
...
begin
  for i:= 1 to dimL do
    begin
      Writeln('primer campo ',v[i].codigoP);
      Writeln('SEGUNDO campo ',v[i].cantidad);
      Writeln('tercero campo ',v[i].dia_venta);
    end;
end;
...
Begin
  ...
  Writeln('assssssssssssaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
  writeln();
  if (dimL = 0) then
